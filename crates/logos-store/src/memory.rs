...
impl MemoryStore {
    ...
    /// Optimization: Returns a `HashSet<&TransactionId>` instead of `HashSet<TransactionId>` to avoid
    /// cloning every `TransactionId` during filtering, reducing memory allocations on hot read paths.
