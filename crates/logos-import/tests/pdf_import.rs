...
#[test]
fn test_parse_slash_date_year_raw_100_does_not_add_2000() {
    let pdf_path = std::env::temp_dir().join(format!("year-100-{}.pdf", std::process::id()));
...
#[test]
fn test_valid_calendar_date_all_months() {
    let pdf_path = std::env::temp_dir().join(format!("all-months-{}.pdf", std::process::id()));

    // Test a valid day in all 12 months to cover the match arms
    std::fs::write(
        &pdf_path,
