...
fn load_transactions(db: &AletheiaDB) -> Result<TransactionLoad, StoreError> {
    ...
    let capacity = txn_node_ids.size_hint().1.unwrap_or(0);
...
fn load_corrections(
    ...
    let mut corrections = Vec::with_capacity(correction_node_ids.size_hint().1.unwrap_or(0));
...
fn load_analytics_artifacts(db: &AletheiaDB) -> Result<ArtifactLoad, StoreError> {
    ...
    let capacity = artifact_node_ids.size_hint().1.unwrap_or(0);
    // Pre-allocate hash map based on known node count to eliminate runtime hashing reallocations.
...
fn load_import_batches(db: &AletheiaDB) -> Result<ImportBatchLoad, StoreError> {
    ...
    let capacity = batch_node_ids.size_hint().1.unwrap_or(0);
...
fn load_statement_lines(
    ...
    let capacity = line_node_ids.size_hint().1.unwrap_or(0);
...
fn load_reconciliation_runs(
    ...
    let capacity = run_node_ids.size_hint().1.unwrap_or(0);
...
fn load_month_closes(
    ...
    let capacity = close_node_ids.size_hint().1.unwrap_or(0);
