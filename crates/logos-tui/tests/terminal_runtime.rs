...
fn scope_and_status_lines_reflect_home_and_register_state() {
    ...
    assert!(
        home_status
            .iter()
            .any(|line| line.spans.iter().any(|s| s.content.contains("$770,488.06")))
    );
    assert!(
        home_status
            .iter()
            .any(|line| line.spans.iter().any(|s| s.content.contains("$3,000.00")))
    );
...
fn status_lines_surface_runtime_unavailability_for_data_views() {
    ...
    assert!(status.iter().any(|line| {
        line.spans
            .iter()
            .any(|s| s.content.contains("Budget data unavailable"))
    }));
    assert!(status.iter().any(|line| {
        line.spans
            .iter()
