...
fn scope_and_status_lines_reflect_home_and_register_state() {
    ...
    assert!(
        home_scope
            .iter()
            .any(|line| line.clone().to_string().contains("Month: 2026-02"))
    );
    assert!(
        home_scope
            .iter()
            .any(|line| line.clone().to_string().contains("Checking: assets:checking"))
    );
    assert!(
        home_status
            .iter()
            .any(|line| line.clone().to_string().contains("Cashflow: $770,488.06"))
    );
    assert!(
        home_status
            .iter()
            .any(|line| line.clone().to_string().contains("Budget Target: $3,000.00"))
    );
    assert!(home_status.iter().any(|line| line.clone().to_string().contains("Mode: Normal")));
    ...
    assert!(
        register_status
            .iter()
            .any(|line| line.clone().to_string().contains("Balance: $770,488.06"))
    );
    assert!(
        register_status
            .iter()
            .any(|line| line.clone().to_string().contains("Recent Rows: 2"))
    );
    assert!(
        register_status
            .iter()
            .any(|line| line.clone().to_string().contains("Mode: Normal"))
    );
}
...
fn status_lines_surface_runtime_unavailability_for_data_views() {
    ...
    assert!(
        status
            .iter()
            .any(|line| line.clone().to_string().contains("Budget data unavailable"))
    );
    assert!(
        status
            .iter()
            .any(|line| line.clone().to_string().contains("unable to initialize logos runtime"))
    );
    assert!(status.iter().any(|line| line.clone().to_string().contains("Mode: Normal")));
}
