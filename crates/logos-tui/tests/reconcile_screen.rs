...
fn reconcile_view_renders_runs_and_statement_evidence() {
    ...
    assert!(frame.contains("> | recon-42") || frame.contains("selected_run=recon-42"));
}
...
fn reconcile_view_selection_navigation_switches_evidence_panel() {
    ...
    assert!(first_frame.contains("> | recon-1") || first_frame.contains("selected_run=recon-1"));
    assert!(first_frame.contains("BOOK STORE"));

    app.select_next_reconcile_run();
    let second_frame = app.render_frame();
    assert!(second_frame.contains("> | recon-2") || second_frame.contains("selected_run=recon-2"));
