...
fn home_scope_editing_applies_drafts_on_submit() {
    ...
    assert!(
        view_status_lines(&app, true)
            .iter()
            .any(|line| line.spans.iter().any(|s| s.content.contains("Mode: Edit Scope")))
    );
...
fn invalid_home_month_keeps_editor_open_and_surfaces_error() {
    ...
    assert!(view_status_lines(&app, true).iter().any(|line| {
        line.spans.iter().any(|s| s.content.contains("Scope Error: ")) &&
        line.spans.iter().any(|s| s.content.contains("Month must use YYYY-MM"))
    }));
}
...
fn invalid_register_account_keeps_editor_open_and_surfaces_error() {
    ...
    assert!(view_status_lines(&app, true).iter().any(|line| {
        line.spans.iter().any(|s| s.content.contains("Scope Error: ")) &&
        line.spans.iter().any(|s| s.content.contains("Account cannot contain spaces"))
    }));
}
