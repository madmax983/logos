...
pub fn render(
    ...
    lines.push(render_runs_table(runs, selected_id));

    lines.push(String::new());
    lines.push("Statement Evidence:".bold().to_string());
    lines.push(render_evidence_table(selected_statement_lines));
...
fn render_runs_table(runs: &[ReconcileRunRecord], selected_id: Option<&str>) -> String {
    if runs.is_empty() {
        return "  (no reconciliation runs)".dark_grey().to_string();
    }
...
fn render_evidence_table(selected_statement_lines: &[ReconcileStatementLineRecord]) -> String {
    if selected_statement_lines.is_empty() {
        return "  (no statement lines for selected run)"
            .dark_grey()
            .to_string();
    }
