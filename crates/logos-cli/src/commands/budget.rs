...
fn render_rsu_plan_table(plan: &RsuBudgetPlan) -> comfy_table::Table {
    ...
    plan_table.add_row(vec![
        ...
        Cell::new(logos_core::currency(plan.conservative_budget_cents())),
        Cell::new(logos_core::currency(plan.fixed_commitments_cents())),
        Cell::new(logos_core::currency(plan.baseline_remaining_cents()))
            .add_attribute(Attribute::Bold),
        ...
        if let Some(scenario) = plan.scenario(key) {
            ...
            scenario_table.add_row(vec![
                Cell::new(scenario_name(key))
                    .fg(color)
                    .add_attribute(Attribute::Bold),
                Cell::new(logos_core::currency(scenario.monthly_income_cents())),
                Cell::new(logos_core::currency(scenario.surplus_cents())),
                Cell::new(logos_core::currency(scenario.reserve_sweep_cents())),
                Cell::new(logos_core::currency(scenario.investing_sweep_cents())),
