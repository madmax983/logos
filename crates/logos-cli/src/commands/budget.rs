...
fn render_budget_set_output(
    ...
    let variance_cell = Cell::new(logos_core::format::currency(variance_cents)).set_alignment(comfy_table::CellAlignment::Right)
        .fg(variance_color)
...
fn render_monte_carlo_output(result: &logos_core::monte_carlo::MonteCarloResult) -> String {
    ...
    table.add_row(vec![
        ...
        Cell::new(logos_core::format::currency(result.p5_cents)).set_alignment(comfy_table::CellAlignment::Right).fg(Color::Red),
    ]);
    table.add_row(vec![
        Cell::new("Median (Expected)")
            .fg(Color::Green)
            .add_attribute(Attribute::Bold),
        Cell::new(logos_core::format::currency(result.median_cents)).set_alignment(comfy_table::CellAlignment::Right)
            .fg(Color::Green)
            .add_attribute(Attribute::Bold),
    ]);
    table.add_row(vec![
        Cell::new("P95 (Optimistic)").fg(Color::Blue),
        Cell::new(logos_core::format::currency(result.p95_cents)).set_alignment(comfy_table::CellAlignment::Right).fg(Color::Blue),
    ]);
