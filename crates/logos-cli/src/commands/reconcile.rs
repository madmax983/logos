...
fn render_month_output(
    ...
    let table = render_single_run_table(
        run,
        Some(month_key),
        Some(checking_account),
        Some(opening_balance_cents),
    );
    table.to_string()
}

fn render_show_output(run: &StoredReconciliationRun) -> String {
    let table = render_single_run_table(run, None, None, None);
    table.to_string()
}

fn render_single_run_table(
    run: &StoredReconciliationRun,
    override_month: Option<&str>,
    override_account: Option<&str>,
    override_opening: Option<i64>,
) -> comfy_table::Table {
    ...
    table.add_row(vec![
        ...
        Cell::new(override_month.unwrap_or_else(|| run.month_key())),
        Cell::new(override_account.unwrap_or_else(|| run.checking_account())),
        Cell::new(crate::format::currency(override_opening.unwrap_or_else(|| run.opening_balance_cents()))),
