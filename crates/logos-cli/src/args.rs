...
fn parse_txn(args: &[String]) -> Result<ParsedArgs, CliError> {
    ...
    match subcommand.as_str() {
        ...
        "add" => {
            ...
            let amount_cents = parse_required_parsed_flag(&args[2..], "--amount-cents")?;
            Ok(ParsedArgs {
...
fn parse_budget(args: &[String]) -> Result<ParsedArgs, CliError> {
    ...
    match subcommand.as_str() {
        ...
        "set" => {
            ...
            let budget_cents =
                parse_optional_parsed_flag(&args[2..], "--budget-cents", DEFAULT_BUDGET_CENTS)?;
        ...
        "rsu-plan" => {
            let month_key = parse_optional_month_flag(&args[2..], "--month")?;
            let quarterly_units = parse_required_parsed_flag(&args[2..], "--quarterly-units")?;
            let days_to_vest = parse_optional_parsed_flag(&args[2..], "--days-to-vest", 45)?;
            let bear_price_cents = parse_required_parsed_flag(&args[2..], "--bear-price-cents")?;
            let base_price_cents = parse_required_parsed_flag(&args[2..], "--base-price-cents")?;
            let bull_price_cents = parse_required_parsed_flag(&args[2..], "--bull-price-cents")?;
            let fixed_commitments_cents =
                parse_optional_parsed_flag(&args[2..], "--fixed-commitments-cents", 0)?;
            let reserve_sweep_pct =
                parse_optional_parsed_flag(&args[2..], "--reserve-sweep-pct", 60)?;
            let investing_sweep_pct =
                parse_optional_parsed_flag(&args[2..], "--investing-sweep-pct", 30)?;
            Ok(ParsedArgs {
...
fn parse_analytics_snapshot(args: &[String]) -> Result<ParsedArgs, CliError> {
    ...
    match action.as_str() {
        "create" => {
            let as_of_valid_time_us = parse_optional_parsed_value(&args[3..], "--as-of-valid-us")?;
            let as_of_tx_time_us = parse_optional_parsed_value(&args[3..], "--as-of-tx-us")?;
            let schema_version = parse_optional_parsed_flag(
                &args[3..],
                "--schema-version",
...
fn parse_import(args: &[String]) -> Result<ParsedArgs, CliError> {
    ...
    match subcommand.as_str() {
        ...
        "csv" => {
            ...
            let timestamp_idx = parse_optional_parsed_flag(&args[2..], "--timestamp-idx", 0)?;
            let amount_idx = parse_optional_parsed_flag(&args[2..], "--amount-idx", 1)?;
            let memo_idx = parse_optional_parsed_flag(&args[2..], "--memo-idx", 2)?;
            let account_idx = parse_optional_parsed_flag(&args[2..], "--account-idx", 3)?;
            let category_idx = parse_optional_parsed_flag(&args[2..], "--category-idx", 4)?;
    ...
    match subcommand.as_str() {
        ...
        "month" => {
            ...
            let opening_balance_cents =
                parse_required_parsed_flag(&args[2..], "--opening-balance-cents")?;
            let closing_balance_cents =
                parse_required_parsed_flag(&args[2..], "--closing-balance-cents")?;
            Ok(ParsedArgs {
...
fn parse_optional_parsed_flag<T: std::str::FromStr>(
    ...
    value.parse::<T>().map_err(|_| CliError::InvalidArgValue {
        flag: flag.to_owned(),
        value,
    })
}
