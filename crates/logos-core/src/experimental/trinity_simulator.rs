...
impl TrinitySimulator {
    ...
    pub fn run(&self, years: u16, paths: u32) -> TrinityResult {
        ...
        let successful_paths = (0..paths)
            .filter(|_| self.simulate_single_path(&mut lcg, years))
            .count();

        #[allow(clippy::cast_precision_loss)]
        let success_rate_f64 = (f64::from(u32::try_from(successful_paths).unwrap_or(0)) / f64::from(paths)) * 100.0;
    ...
    fn simulate_single_path(&self, lcg: &mut Lcg, years: u16) -> bool {
