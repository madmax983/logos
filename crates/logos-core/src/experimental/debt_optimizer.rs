...
impl DebtOptimizer {
    ...
    pub fn simulate(&self, strategy: PayoffStrategy) -> PayoffResult {
        ...
        while current_debts.iter().any(|d| d.balance_cents > 0) {
            ...
            Self::sort_debts(&mut current_debts, strategy);
            Self::allocate_extra_payments(&mut current_debts, &mut remaining_cash);
        }
    ...
    fn sort_debts(debts: &mut [Debt], strategy: PayoffStrategy) {
        match strategy {
    ...
    fn allocate_extra_payments(debts: &mut [Debt], remaining_cash: &mut i64) {
        for debt in debts {
