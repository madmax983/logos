...
impl DebtOptimizer {
    ...
    pub fn simulate(&self, strategy: PayoffStrategy) -> PayoffResult {
        ...
        while current_debts.iter().any(|d| d.balance_cents > 0) {
            ...
            if remaining_cash > 0 {
                Self::pay_extra(&mut current_debts, remaining_cash, strategy);
            }
        }
    ...
    fn pay_minimums(debts: &mut [Debt], mut available_cash: i64) -> i64 {
        for debt in debts {
            if debt.balance_cents > 0 {
                let payment = std::cmp::min(debt.balance_cents, debt.min_payment_cents);
                let actual_payment = std::cmp::min(payment, available_cash);
    ...
    fn pay_extra(debts: &mut [Debt], mut remaining_cash: i64, strategy: PayoffStrategy) {
        match strategy {
