...
impl DebtOptimizer {
    ...
    pub fn simulate(&self, strategy: PayoffStrategy) -> PayoffResult {
        ...
        while current_debts.iter().any(|d| d.balance_cents > 0) {
            ...
            Self::sort_debts_for_strategy(&mut current_debts, strategy);
            Self::pay_extra(&mut current_debts, remaining_cash);
        }
    ...
    fn sort_debts_for_strategy(debts: &mut [Debt], strategy: PayoffStrategy) {
        match strategy {
    ...
    fn pay_extra(debts: &mut [Debt], mut remaining_cash: i64) {
        for debt in debts {
