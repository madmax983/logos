...
## 2024-06-19 - CashflowProjector panics on arithmetic overflow
**Learning:** Unguarded `+=` operator in `CashflowProjector::project_balances` caused arithmetic overflow panic when simulating large recurring cashflows.
**Action:** Use `.saturating_add()` to prevent panics when aggregating financial amounts.
