...
**[Diesel eq_any Queries]**
**Learning:** `Diesel::eq_any` accepts any generic `IntoIterator`. It is unnecessary to collect mapped item identifiers (like strings or string slices) into an intermediate `Vec<&str>` just to feed them into `eq_any`.
**Action:** Always check if a function accepts `IntoIterator` before eagerly calling `.collect()`. Pass `.iter().map(...)` directly.
