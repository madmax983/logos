...
## Avoid intermediate Vec allocations for diesel IN clauses
**Learning:** `diesel`'s `.eq_any()` accepts iterators, meaning we don't have to `collect()` intermediate vectors when building IN clauses for filtering records.
**Action:** Remove `.collect()` calls and directly pass iterators like `rows.iter().map(...)` or `transaction_ids.iter().map(...)` directly to `.eq_any()` to avoid creating an intermediate `Vec`.
## 2024-05-19 - Pre-allocate inner loop primitives
**Learning:** `Vec::new()` and `String::new()` inside hot loops (like character-by-character CSV parsing) result in many small heap allocations.
**Action:** When parsing lines, use `Vec::with_capacity` and `String::with_capacity` based on typical data sizes to drastically reduce the number of heap allocations.
