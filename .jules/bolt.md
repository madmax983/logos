...
## 2026-05-26 - Reduce unconditional clone in loops by iterating over references
**Learning:** Found several places where `.clone()` was being used unnecessarily inside iteration over loops or when initializing vectors. Removing these calls avoids unnecessary intermediate memory allocations and cloning. Using references `&T` or iterating with `.into_iter()` directly where possible avoids fighting the borrow checker.
**Action:** Remove `.clone()` and use references directly when traversing data to improve efficiency and reduce memory allocations.
