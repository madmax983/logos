...
## YYYY-MM-DD - Reduce Iterator Collections
**Learning:** Found several places where `.iter().map(...).collect::<Vec<_>>()` and `.filter_map(...).collect::<Vec<_>>()` were immediately passed into functions that only required references or could consume an iterator. By changing `ensure_transactions_exist` to take an `impl Iterator<Item = &'a str>` instead of `&[TransactionId]`, we eliminated intermediate allocations, deep copies (via `.cloned()`), and the need to instantiate large memory structures when a streaming implementation suffices. However, beware of passing `Vec<TransactionId>` iterators to `statement_line_ids_for_transaction_ids` when `eq_any` is used, because Diesel requires a slice for `.eq_any(&[T])`.
**Action:** When a function accepts a slice just to loop over it for existence checks, change the signature to `impl Iterator<Item = &T>` or `impl Iterator<Item = &'a str>`. Pass the `.iter().map(...)` directly.
