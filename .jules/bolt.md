...
## YYYY-MM-DD - Pre-allocate capacity for domain objects with mathematically constrained minimum sizes
**Learning:** Double-entry accounting requires a minimum of 2 entries (debit and credit). Using `Vec::new()` causes an immediate heap reallocation upon population.
**Action:** Always pre-allocate capacity using `Vec::with_capacity(N)` for domain objects with known minimum constraints to avoid unnecessary heap allocations.
