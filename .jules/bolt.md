...
**Eliminated Intermediate Vector in Income Router**
**Learning:** Intermediate vectors used purely to hold data before a final reduction or sweep can often be eliminated by replacing a single O(N) allocating pass with two O(N) non-allocating passes (e.g. pre-computing the sweep value first).
**Action:** Look for `.push()` loops followed by immediate iteration, and see if the intermediate state can be calculated on the fly or via a quick pre-pass.
