...
**Refactoring God Functions in Monte Carlo & Simulators**
**Learning:** Functions like `run` and `simulate` in simulators can grow very long and deeply nested because of the multiple loops required (outer loop for paths, inner loop for iterations).
**Action:** Extract the inner path evaluation loop into a private helper method (e.g., `simulate_path`) that takes `&mut Lcg` and iteration counts. This reduces the primary `run` method to just managing the collection and summary statistics, significantly improving readability.
