...
**Refactoring Simulation God Functions**
**Learning:** Nested loops (paths * iterations) inside simulation run functions create Pyramid of Doom and increase cognitive load. Leaving them in one function makes `run` hard to read.
**Action:** Extract the inner iteration loop into a private `simulate_path` helper method that accepts the PRNG state (`&mut Lcg`) and required variables. This reduces function length, improves readability, and keeps PRNG states properly advancing.
