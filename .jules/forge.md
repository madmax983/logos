...
**Refactoring Simulation Loops**
**Learning:** Inner simulation loops in `monte_carlo.rs` and `trinity_simulator.rs` were deeply nested and duplicated the same `Lcg` structural definitions, violating DRY and creating "God Functions". When extracting these loops to `simulate_path`, it's critical to hoist loop-invariant math (like calculating `monthly_mean`) out of the method to avoid massive performance regressions.
**Action:** Extract identically duplicated structs into shared modules (e.g. `lcg.rs`). When extracting loop bodies to helper methods, pass loop-invariant pre-computed variables as arguments instead of recalculating them inside the helper method on every single path.
