...
**Learning:** Functions like `DebtOptimizer::simulate` often contain large `while` loops executing multiple complex phases (applying interest, paying minimums, sorting, paying extra). These create deeply nested Pyramids of Doom and are hard to read at a high level.
**Action:** Extract the distinct loop phases into private static helper functions that take mutable slices (e.g., `&mut [Debt]`). This flattens the nesting and allows the main loop to read like a high-level orchestrator.
