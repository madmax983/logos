...
**Extracting simulation phases in loops**
**Learning:** Functions like `DebtOptimizer::simulate` that run financial simulations often contain long `while` loops doing multiple distinct phases (apply interest, pay minimums, pay extra strategy). This causes the function to exceed readability lengths.
**Action:** Extract each phase of the simulation loop into its own `fn` helper (e.g. `apply_interest`, `pay_minimums`) taking a mutable slice `&mut [Debt]`. This drastically flattens the simulation logic and makes the loop trivially readable.
