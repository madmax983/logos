...
**Refactoring God Functions via Extraction**
**Learning:** Functions like `ascend` in `fire_ascent.rs`, `run` in monte_carlo and trinity_simulator, and `parse_csv_columns` in `csv.rs` were > 60 lines and nested, making them "God Functions". Refactoring them required extracting the nested `for` loops or `match` arms into named helper functions.
**Action:** Extract loop bodies (e.g. `simulate_path`) or match arms (e.g. `handle_unquoted_char`) into helper functions. Use `const fn` for extracted methods if they don't capture non-const environment to keep clippy happy.
