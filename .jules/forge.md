...
**Refactoring God Functions in Experimental modules**
**Learning:** `DebtOptimizer::simulate` and `FireAscentSimulator::ascend` contained large blocks of logic inline (e.g., applying interest, paying minimums, allocating cash, building milestones).
**Action:** Extracted distinct logic phases into smaller, named helper functions within the struct's implementation block. When dealing with large loops in financial simulations, prefer passing mutable slices (e.g., `&mut [Debt]`) to these helpers rather than keeping multi-phase `while` loops.
