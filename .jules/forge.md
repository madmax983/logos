...
**Refactoring `debt_optimizer.rs`'s God Function**
**Learning:** `crates/logos-core/src/experimental/debt_optimizer.rs` contained `simulate`, which was a long function that looped through multiple strategies, handled minimum payments, interest calculations, and strategy execution all inline, causing it to exceed 60 lines and include deep nesting.
**Action:** Extract the loop body logic into cleanly named helper functions (e.g., `apply_monthly_interest`, `pay_minimums`, `pay_extra_according_to_strategy`). Pass the required variables by mutable reference to maintain state mutation without introducing ownership overhead or architectural changes.
