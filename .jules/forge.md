...
**Extracted Helper Functions from God Loops**
**Learning:** Simulation loops (like those in `DebtOptimizer`, `MonteCarloProjector`, and `TrinitySimulator`) can easily become long "God Functions" containing nested logic. The workspace also enforces strict clippy rules like `clippy::trivially-copy-pass-by-ref`, meaning if a struct parameter (like `InflationProjector` which is just an `f64`) is extracted into a helper, it must be passed by value if it is trivially copyable.
**Action:** Extract deep nesting and inner logic out of loops into separate helper functions. For small structs, pass them by value.
