...
**Extract complex loop logic into typed helpers**
**Learning:** Loops that perform multiple independent steps on the same data structures (like `simulate` applying interest, then paying minimums, then allocating extra payments) grow out of control and obscure the algorithm.
**Action:** Break loop logic into private, descriptive static helper functions (`apply_interest`, `pay_minimums`, `allocate_extra_payments`) that pass the mutable state along. Use saturating math inside for financial safety.
